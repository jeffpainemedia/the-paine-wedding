...
export default function ConnectionsGate({ children }: ConnectionsGateProps) {
    ...
    useEffect(() => {
        const interval = window.setInterval(() => {
            setRemaining(getTimeRemaining(getConnectionsUnlockDate()));
        }, 1000);
    ...
    return (
        <div className="relative overflow-hidden rounded-[2rem] border border-primary/10 bg-[linear-gradient(150deg,#fffaf4_0%,#f2ece3_52%,#e6eef2_100%)] p-8 shadow-[0_20px_60px_rgba(20,42,68,0.10)] md:p-10">
            ...
            <h2 className="relative mt-4 font-heading text-4xl text-primary">Connected Opens April 12</h2>
            <p className="relative mt-4 max-w-2xl leading-relaxed text-text-secondary">
                This puzzle goes live on {CONNECTIONS_UNLOCK_LABEL} — a new word grouping challenge every day.
            </p>
